...
// SpiderOutput 表示 Spider 回调的输出，可以是 Request 或 Item。
...
func (o SpiderOutput) IsRequest() bool {
	return o.Request != nil
}

// IsItem 检查输出是否为 Item。
func (o SpiderOutput) IsItem() bool {
	return o.Item != nil
}

// CallbackFunc 定义 Spider 回调函数类型。
// 接收 context 和 Response，返回 SpiderOutput 切片和可能的错误。
type CallbackFunc func(ctx context.Context, response *scrapy_http.Response) ([]SpiderOutput, error)

// ErrbackFunc 定义 Spider 错误回调函数类型。
// 接收 context、错误和原始请求，返回 SpiderOutput 切片和可能的错误。
type ErrbackFunc func(ctx context.Context, err error, request *scrapy_http.Request) ([]SpiderOutput, error)
...
type Spider interface {
	...
	Start(ctx context.Context) <-chan SpiderOutput

	// Parse 是默认的响应回调函数。
	// 当 Request.Callback 为 nil 时，Engine 使用此方法处理响应。
	Parse(ctx context.Context, response *scrapy_http.Response) ([]SpiderOutput, error)

	// CustomSettings 返回 Spider 级别的配置覆盖（可选）。
	// 返回 nil 表示不覆盖任何配置。
	CustomSettings() *SpiderSettings
...
func (s *BaseSpider) Start(ctx context.Context) <-chan SpiderOutput {
	ch := make(chan SpiderOutput)
	go func() {
		defer close(ch)
		for _, rawURL := range s.StartURLs {
			...
			case <-ctx.Done():
				return
			case ch <- SpiderOutput{Request: req}:
			}
		}
	}()
	return ch
...
func (s *BaseSpider) Parse(ctx context.Context, response *scrapy_http.Response) ([]SpiderOutput, error) {
	return nil, nil
}

// CustomSettings 返回 nil（不覆盖任何配置）。
func (s *BaseSpider) CustomSettings() *SpiderSettings {
	return nil
}

// Closed 在 Spider 关闭时调用（空实现）。
func (s *BaseSpider) Closed(reason string) {
}
