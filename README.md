...
- ✅ **Response** — 支持 Text/JSON 解析、URLJoin 相对路径解析、Follow 链接跟踪
...
- ✅ **Spider 级别配置** — 通过 `CustomSettings()` 返回类型安全的 `SpiderSettings` 结构体
- ✅ **配置冻结** — 支持 Freeze 防止运行时意外修改
- ✅ **组件优先级字典** — 支持 `_BASE` + 用户配置合并，负数优先级表示禁用

### 统计与日志

- ✅ **MemoryStatsCollector** — 基于内存的统计收集，Spider 关闭时自动 Dump
...
// 解析响应
func (s *MySpider) Parse(ctx context.Context, response *scrapy_http.Response) ([]spider.SpiderOutput, error) {
...
```go
func (s *MySpider) CustomSettings() *spider.SpiderSettings {
...
| **中间件体系** | 下载器中间件（ProcessRequest/ProcessResponse/ProcessException）和 Spider 中间件（ProcessSpiderInput/ProcessSpiderOutput/ProcessSpiderException）接口一致 |
...
| **Spider 配置** | `custom_settings` 返回 dict | `CustomSettings()` 返回类型安全的 `SpiderSettings` 结构体 |
| **Request 构建** | 关键字参数 `Request(url, callback=self.parse)` | Functional Options `NewRequest(url, WithCallback(s.Parse))` |
| **外部依赖** | Twisted、lxml、cssselect 等 | 零外部依赖，仅使用 Go 标准库 |
| **HTML 解析** | 内置 Selector（CSS/XPath） | 不内置，推荐使用 goquery 等第三方库 |
| **部署** | 需要 Python 运行时 | 编译为单一二进制文件，无运行时依赖 |
| **内存占用** | 较高（Python 对象开销） | 较低（Go 值类型 + 紧凑内存布局） |
| **Spider 定义** | 类继承 `class MySpider(scrapy.Spider)` | 接口实现 + 结构体嵌入 `BaseSpider` |

## 当前版本限制

作为 MVP 版本，以下功能尚未实现，计划在后续版本中逐步完善：

### 未实现的功能

| 功能 | 说明 |
|------|------|
| **CSS/XPath 选择器** | 不内置 HTML 选择器，需使用 goquery 等第三方库 |
| **磁盘队列** | Scheduler 仅支持内存队列，不支持磁盘持久化（JOBDIR） |
| **HTTP 缓存** | `HTTPCACHE_ENABLED` 配置已预留，但缓存中间件未实现 |
| **Cookies 管理** | `COOKIES_ENABLED` 配置已预留，但 CookiesMiddleware 未实现 |
...
| **HTTP 压缩** | `COMPRESSION_ENABLED` 配置已预留，但压缩中间件未实现 |
...
- **HTML 解析**：框架不内置 HTML 解析能力，示例中使用简单的字符串匹配，生产环境建议使用 [goquery](https://github.com/PuerkitoBio/goquery)
