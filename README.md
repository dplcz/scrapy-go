...
- **Scheduler** — 基于内存优先级队列 + 磁盘队列的请求调度，支持断点续爬（`JOBDIR`）
...
- **`scrapy-go startproject <name>`** — 创建完整的项目骨架（main.go / settings.go / middlewares.go / pipelines.go / items.go / go.mod / scrapy-go.toml）
