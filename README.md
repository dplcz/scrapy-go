...
| QPS（64 并发） | ~12,500 req/s | 本地服务器，最小响应 |
| 系统内存（10 万请求） | ~151 MB | 含 Go 运行时开销 |
...
|------|--------------|--------|-----------|-------------|
| raw net/http | ~17,700 | 1.00x | ~12 KB | 88% |
| Colly 风格 | ~16,500 | 1.07x | ~9 KB | 92% |
| Ferret 风格 | ~13,700 | 1.29x | ~9 KB | 83% |
| **scrapy-go** | **~15,500** | **1.14x** | ~14 KB | **89%** |

> scrapy-go 在提供完整框架功能（调度器、中间件链、信号系统、统计等）的前提下，
> QPS 开销仅为 raw net/http 的 ~1.14x，在有网络延迟的真实场景中效率与裸 HTTP 客户端持平。
...
- 📊 **性能验证** — QPS ~17,900（16 并发），内存 ~159MB（10 万请求），框架开销仅 1.14x
...
- 🔄 **框架对比测试** — 4 种框架（raw net/http、Colly 风格、Ferret 风格、scrapy-go）× 4 个并发级别的 QPS/内存矩阵对比
- 📊 **对比报告** — 格式化输出 QPS 表、框架开销比表、内存分配表
- ✅ **开销验收** — scrapy-go 框架开销仅 ~1.4x（验收标准 < 3.3x），绝对 QPS >= 5000
- ⏱️ **延迟场景** — 10ms 延迟下 scrapy-go 并发调度效率 89%，与裸 HTTP 客户端持平
